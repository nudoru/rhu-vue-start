  ...
  <table :class="cls()">
    ...
    <tbody>
      <tr v-for="(rowData, ridx) in tableData" :key="ridx">
        <td v-for="(col, cidx) in columnData" :key="cidx">
  ...
  methods: {
    ...
    headerIcon(col) {
      ...
      if (col.sortable && col.sorted && col.sortDirection === -1) {
        return "fas fa-sort-up sort-icon";
      } else if (col.sortable && col.sorted && col.sortDirection === 1) {
        return "fas fa-sort-down sort-icon";
    ...
    applyDefaultSort(data) {
      ...
      let firstSortHeader = this.columns.reduce((acc, x, idx) => {
        if (x.sorted && acc === -1) {
          acc = idx;
        }
        return acc;
      ...
      if (firstSortHeader !== -1) {
        return this.applySort(this.columns[firstSortHeader], data);
      }
      return data;
    },
    onSortHeaderClick(colIdx) {
      let column = this.columnData[colIdx];
      if (column.sorted) {
