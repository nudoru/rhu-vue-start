...
export const getElStyleProp = memoizeWith(
  curry((el, prop) => getElStyle(el).getPropertyValue(prop))
);
...
export const createElementSVG = (tag) =>
...
export const wrapChildren = (parentNode, wrapperNode) => {
  let children = parentNode.querySelectorAll('*');
  children.forEach((c) => {
    wrapperNode.appendChild(c);
  });
  parentNode.insertBefore(wrapperNode, parentNode.firstChild);
};
